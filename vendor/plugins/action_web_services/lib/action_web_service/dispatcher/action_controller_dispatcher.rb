    ...
    module ActionController # :nodoc:
      ...
      module InstanceMethods # :nodoc:
        private
          def dispatch_web_service_request
            if request.get?
              render(:text=>'GET not supported 500 GET not supported')
              return
            end
            exception = nil
            begin
          ...
          def send_web_service_error_response(ws_request, exception)
            if ws_request
              ...
              render(:text=>"Internal protocol error: #{message}#{backtrace}, 500 Internal Protocol Error")
            end
          end
      ...
      module WsdlAction # :nodoc:
        ...
        def wsdl
          ...
          when :post
            render(:text=>'POST not supported, 500 POST not supported')
          end
        end
        ...
        private
          def base_uri
            ...
            relative_url_root = self.class.relative_url_root.to_s
                        ...
                        xm.xsd(:complexType, 'name' => binding.type_name) do
                          xm.xsd(:all) do
                                binding.type.each_member do |name, type|
                                    ...
                                    xm.xsd(:element, 'name' => name, 
                                                     'type' => b.qualified_type_name('typens'),
                                                     'default'=>b.type.default,
                                                     'nillable'=>b.type.nillable)
                                end                            
                          end
                        end
                      end
